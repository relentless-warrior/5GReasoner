    ...
    for i in range(len(lines)):
        ...
        #//state variables: rrc_sec_ctx_exist{true, false}<false>;
            ...
            for v in vars:
                ...
                print 'IK' + fsm_label+ state_var_label
        ...
        #     rrc_setup_req, rrc_sec_ctx_exist=false"];
            ...
            if 'ue' in fsm_label.lower() and 'rrc' in fsm_label.lower():
                '''
                if (i for i in condition if i in out_msgs):
                    flag = 1
                    condition +=' & chan_UE_nas_rrc = UE_ul_info_transfer'
                    new_transition = Transition(transition_label, start_state, end_state, condition, actions)
                ...
                if (i for i in actions if i.action_label in in_msgs):
                    flag = 1
                    if inner_flag == 0:
                        new_transition = Transition(transition_label, start_state, end_state, condition, actions)
                        filewrite(fw, transition_label, start_state, end_state, condition, actions)
                        smv_transitions.append(new_transition)
                    channel_name = actions[0].channel
                    del actions[:]
                    transition_counter = transition_counter + 1
                    transition_label = fsm_label + "_T" + str(transition_counter)
                    con ="chan_UE_nas_rrc = ul_info_transfer"
                    act = Action("ul_info_transfer",channel_name)
                    actions.append(act)
                    act = Action("chan_UE_nas_rrc=null",channel_name)
                    actions.append(act)
                    new_transition = Transition(transition_label, end_state, end_state, con, actions)
                    filewrite(fw, transition_label, start_state, end_state, con, actions)
                    smv_transitions.append(new_transition)


                    transition_counter = transition_counter + 1
                    transition_label = fsm_label + "_T" + str(transition_counter)
                    con = "dl_info_transfer"
                    act = Action("chan_UE_rrc_nas=UE_dl_info_transfer",channel_name)
                    actions.append(act)
                    new_transition = Transition(transition_label, end_state, end_state, con, actions)
                    filewrite(fw, transition_label, start_state, end_state, con, actions)
                    smv_transitions.append(new_transition)
                ...
                ''''
                if (i for i in condition if i in env_vars):
                    condition+=' & chan_UE_nas_rrc=null'
                    if (len(actions) == 1 and str(actions[0].action_label) is "null_action"):
                        actions[0].action_label = "chan_AMF_nas_rrc = UE_ul_info_transfer"
                    else:
                        act = Action("chan_AMF_nas_rrc = UE_ul_info_transfer",actions[0].channel)
                        actions.append(act)
                '''
                '''
                ...
                '''
                #fw.write("\ncond"+str(cond_tokens)+"\n")
                flag = 0
                for i in in_msgs:
                    if (i in condition):
                        for j in out_msgs:
                                for k in actions:
                                    if(j in k.action_label):
                                        flag = 1
                if flag == 1:
                    condition += ' & chan_UE_rrc_nas=UE_dl_info_transfer'
                    condition += ' & chan_UE_nas_rrc=null'
                    if (len(actions) == 1 and str(actions[0].action_label) is "null_action"):
                        actions[0].action_label = "chan_UE_nas_rrc=UE_ul_info_transfer"
                        actions[0].channel.channel_label = 'internal'
                        c = Channel("internal", actions[0].channel.start, actions[0].channel.end)
                        act = Action("chan_UE_rrc_nas=null", c)
                        actions.append(act)
                    else:
                        c = Channel("internal",actions[0].channel.start,actions[0].channel.end)
                        act = Action("chan_UE_nas_rrc=UE_ul_info_transfer", c)
                        actions.append(act)
                        act = Action("chan_UE_rrc_nas=null", c)
                        actions.append(act)
                ...
                '''
                if (i for i in condition if i in in_msgs):
                    flag = 1
                    if(len(actions)==1 and str(actions[0].action_label) is "null_action"):
                        actions[0].action_label = "chan_AMF_rrc_nas = AMF_ul_info_transfer"
                    else:
                        act = Action("chan_AMF_rrc_nas = AMF_ul_info_transfer",actions[0].channel)
                        actions.append(act)
                    new_transition = Transition(transition_label, start_state, end_state, condition, actions)
                    filewrite(fw, transition_label, start_state, end_state, condition, actions)
                    smv_transitions.append(new_transition)
                    ...
                    transition_counter = transition_counter + 1
                    transition_label = fsm_label + "_T" + str(transition_counter)
                    con = "ul_info_transfer"
                    act = Action("chan_AMF_rrc_nas = AMF_ul_info_transfer",channel_name)
                    actions.append(act)
                    new_transition = Transition(transition_label, end_state, end_state, con, actions)
                    filewrite(fw, transition_label, start_state, end_state, con, actions)
                    smv_transitions.append(new_transition)
                    ...
                    transition_counter = transition_counter + 1
                    transition_label = fsm_label + "_T" + str(transition_counter)
                    con = "chan_AMF_nas_rrc = AMF_dl_info_transfer"
                    act = Action("dl_info_transfer",channel_name)
                    actions.append(act)
                    act = Action("chan_AMF_nas_rrc = null",channel_name)
                    actions.append(act)
                    new_transition = Transition(transition_label, end_state, end_state, con, actions)
                    filewrite(fw, transition_label, start_state, end_state, con, actions)
                    smv_transitions.append(new_transition)
                ...
                if flag == 1:
                    condition += ' & chan_AMF_rrc_nas=AMF_ul_info_transfer'
                    condition += ' & chan_AMF_nas_rrc=null'
                    if (len(actions) == 1 and str(actions[0].action_label) is "null_action"):
                        actions[0].action_label = "chan_AMF_nas_rrc=AMF_dl_info_transfer"
                        actions[0].channel.channel_label = 'internal'
                        c = Channel("internal", actions[0].channel.start, actions[0].channel.end)
                        act = Action("chan_AMF_rrc_nas=null", c)
                        actions.append(act)
                    else:
                        c = Channel("internal", actions[0].channel.start, actions[0].channel.end)
                        act = Action("chan_AMF_nas_rrc=AMF_dl_info_transfer", c)
                        actions.append(act)
                        act = Action("chan_AMF_rrc_nas=null", c)
                        actions.append(act)
                ...
                if flag != 1:
                    for i in in_msgs:
                        if (i in condition):
                                condition+= ' & chan_AMF_rrc_nas=AMF_ul_info_transfer'
                                if (len(actions) == 1 and str(actions[0].action_label) is "null_action"):
                                    actions[0].action_label = "chan_AMF_rrc_nas=null"
                                    actions[0].channel.channel_label = 'internal'
                                else:
                                    c = Channel("internal", actions[0].channel.start, actions[0].channel.end)
                                    act = Action("chan_AMF_rrc_nas=null",c)
...
def dump_actions(file, fsms):
    for fsm in fsms:
        ...
        if 'null_action' not in action_labels:
            action_labels.append('null_action')
...
def create_vars_combined(vars_client1, vars_client2, vars_server1, vars_server2):
    vars = []
    for var in vars_client1:
        vars.append(var)
    for var in vars_client2:
        vars.append(var)
    for var in vars_server1:
        vars.append(var)
    for var in vars_server2:
        vars.append(var)
    #vars_unique = list(set([var.varname for var in vars]))
    ''''
    for v in vars:
        if(str(v.varname) in "chan_UE_rrc_nas"):
            vars.remove(v)
            break
    for v in vars:
        if (str(v.varname) in "chan_UE_nas_rrc"):
            vars.remove(v)
            break
    for v in vars:
        if (str(v.varname) in "chan_AMF_rrc_nas"):
            vars.remove(v)
            break
    for v in vars:
        if (str(v.varname) in "chan_AMF_nas_rrc"):
            vars.remove(v)
            break
    for v in vars:
        if (str(v.varname) in "paging_timer_expired"):
            vars.remove(v)
            break
    '''''
    var_unique = []
    var_unique.append(vars[0])
...
def create_seq_nums(client_seq_nums, server_seq_nums):
    ...
    for seq_num in server_seq_nums:
        seq_nums.append(seq_num)
    return seq_nums
def create_seq_nums_combined(client1_seq_nums, client2_seq_nums, server1_seq_nums,server2_seq_nums):
    seq_nums = []
    for seq_num in client1_seq_nums:
        seq_nums.append(seq_num)
    for seq_num in client2_seq_nums:
        seq_nums.append(seq_num)
    for seq_num in server1_seq_nums:
        seq_nums.append(seq_num)
    for seq_num in server2_seq_nums:
        seq_nums.append(seq_num)
    return seq_nums
...
def create_fsms_combined (fsm_client1,fsm_client2, fsm_server1,fsm_server2):
    fsms = []
    fsms.append(fsm_client1)
    fsms.append(fsm_client2)
    fsms.append(fsm_server1)
    fsms.append(fsm_server2)
    return fsms
...
def create_channels_combined(client1_fsm_label, client2_fsm_label, server1_fsm_label,server2_fsm_label):
    channels = []
    injective_adversaries = []
    #if 'ue' in client1_fsm_label.lower() and 'rrc' in client_fsm_label.lower():
    channel1 = Channel('chan_UB', 'UE_RRC', 'BS_RRC')
    channel2 = Channel('chan_BU', 'BS_RRC', 'UE_RRC')
    channel3 = Channel('chan_UA', 'UE_NAS', 'AMF_NAS')
    channel4 = Channel('chan_AU', 'AMF_NAS', 'UE_NAS')
    channels.append(channel1)
    channels.append(channel2)
    ...
    injective_adversary1 = InjectiveAdversary('inj_adv_UB', 'chan_UB' , True)
    injective_adversary2 = InjectiveAdversary('inj_adv_BU', 'chan_BU', True)
    injective_adversary3 = InjectiveAdversary('inj_adv_UA', 'chan_UA', True)
    injective_adversary4 = InjectiveAdversary('inj_adv_AU', 'chan_AU', True)
    injective_adversaries.append(injective_adversary1)
    injective_adversaries.append(injective_adversary2)
    injective_adversaries.append(injective_adversary3)
    injective_adversaries.append(injective_adversary4)
    #if 'ue' in client_fsm_label.lower() and 'nas' in client_fsm_label.lower():
    return channels, injective_adversaries
...
def compile_manual_checks_combined(client1_checks, client2_checks, server1_checks, server2_checks):
    checks = []
    if client1_checks is not None and len(client1_checks)!=0:
        for c in client1_checks:
            checks.append(c)
    if client2_checks is not None and len(client2_checks) != 0:
        for c in client2_checks:
            checks.append(c)
    if server1_checks is not None and len(server1_checks) != 0:
        for c in server1_checks:
            checks.append(c)
    if server2_checks is not None and len(server2_checks) != 0:
        for c in server2_checks:
            checks.append(c)
    return checks
...
def main_combined(client1_in,client2_in, server1_in, server2_in, outputFile):

    #if 'ue' in client_in.lower():
    #if 'rrc' in client_in.lower():
    client1_fsm_label = 'UE_RRC'
    client2_fsm_label = 'UE_NAS'
    server1_fsm_label = 'BS_RRC'
    server2_fsm_label = 'AMF_NAS'
        #elif 'nas' in client_in.lower():
            #client_fsm_label = 'UE_NAS'
    #if 'bs' in server_in.lower():
        #server_fsm_label = 'BS_RRC'
    ...
    parsing_result_client1 = parseDOT(client1_in, client1_fsm_label) # return (env_vars, state_vars, seq_vars, smv_vars, smv_seq_nums, fsm, smv_manual_checks)
    parsing_result_client2 = parseDOT(client2_in,client2_fsm_label)  # return (env_vars, state_vars, seq_vars, smv_vars, smv_seq_nums, fsm, smv_manual_checks)
    parsing_result_server1 = parseDOT(server1_in, server1_fsm_label) # return (env_vars, state_vars, seq_vars, smv_vars, smv_seq_nums, fsm, smv_manual_checks)
    parsing_result_server2 = parseDOT(server2_in, server2_fsm_label)  # return (env_vars, state_vars, seq_vars, smv_vars, smv_seq_nums, fsm, smv_manual_checks)

    vars = create_vars_combined(parsing_result_client1[3],parsing_result_client2[3], parsing_result_server1[3],parsing_result_server2[3]) # seq_vars
    seq_nums = create_seq_nums_combined(parsing_result_client1[4],parsing_result_client2[4], parsing_result_server1[4],parsing_result_server2[4]) # smv_vars
    fsms = create_fsms_combined(parsing_result_client1[5],parsing_result_client2[5], parsing_result_server1[5],parsing_result_server2[5])

    channels, injective_adversaries = create_channels_combined(client1_fsm_label,client2_fsm_label, server1_fsm_label,server2_fsm_label)
    manual_checks = compile_manual_checks_combined(parsing_result_client1[6],parsing_result_client2[6], parsing_result_server1[6],parsing_result_server2[6])
    print 'manual_checks = ', manual_checks
    f = open(args.outputFile, "w")
    f.write("MODULE main\n")
    dump_variables(f, vars, injective_adversaries)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-client1', dest='client1_dot_file', default="FSM/RRC/UE-RRC-5G.dot",
                        help="UE RRC dot file to read")
    parser.add_argument('-server1', dest='server1_dot_file', default="FSM/RRC/BS-RRC-5G.dot",
                        help="gNB RRC dot file to read")
    #parser.add_argument('-o', dest='outputFile', default="5G-RRC.smv", help="smv file to write")

    parser.add_argument('-client2', dest='client2_dot_file', default="FSM/NAS/UE-NAS-5G.dot", help="UE NAS dot file to read")
    parser.add_argument('-server2', dest='server2_dot_file', default="FSM/NAS/AMF-NAS-5G.dot",
                         help="AMF NAS dot file to read")
    parser.add_argument('-o', dest='outputFile', default="combined_layers.smv", help="smv file to write")

    args = parser.parse_args()
    main_combined(args.client1_dot_file,args.client2_dot_file ,args.server1_dot_file,args.server2_dot_file, args.outputFile)
